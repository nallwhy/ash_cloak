...
defmodule AshCloak.EncryptNilTest do
  ...
  describe "encrypt_nil? true (default)" do
    test "encrypts nil into a non-nil ciphertext" do
      created =
        AshCloak.Test.Resource
        |> Ash.Changeset.for_create(:create, %{encrypted: nil})
        |> Ash.create!()

      # a nil value is still encrypted, so the backing attribute holds a ciphertext ...
  ...
  describe "encrypt_nil? false" do
    test "stores nil in the backing attribute on create and round-trips back to nil" do
      created =
        AshCloak.Test.NilResource
        |> Ash.Changeset.for_create(:create, %{encrypted: nil})
        |> Ash.create!()

      # the backing attribute is nil rather than a ciphertext
      assert created.encrypted_encrypted == nil

      # and the decrypt calculation maps the nil backing value back to nil
    ...
    test "clears the backing attribute to nil on update" do
      created =
        AshCloak.Test.NilResource
        |> Ash.Changeset.for_create(:create, %{encrypted: 12})
        |> Ash.create!()
    ...
    test "clears the backing attribute to nil on atomic/bulk update" do
      created =
        AshCloak.Test.NilResource
        |> Ash.Changeset.for_create(:create, %{encrypted: 12})
        |> Ash.create!()
